...
export default function ChatPageCustomer() {
    const router = useRouter();
    const userId = 1; // شناسه کاربر خود را اینجا قرار دهید
    ...
    const [openMenu, setOpenMenu] = useState(false); // باز کردن منوی لیست کاربران
    ...
    return (
        <section className='relative flex'>
            <div className='bg-green-900 h-screen p-2 pt-14 overflow-hidden w-full'>
                ...
                <section className='h-full flex flex-col relative'>
                                ...
                                <div key={index} className="flex justify-between items-start ">
                                    {msg.userId === String(userId) ? (
                                        <div className={`flex py-2 ml-5 break-words overflow-hidden`}>
                                            <div className={`bg-green-300 md:ml-20 lg:ml-52 pl-6 pr-10 pb-6 py-1 rounded-3xl rounded-br-none shadow-md relative min-w-16 w-full ${index === messages.length - 1 && 'pm-animation'}`}>
                    ...
                    <div className="sticky left-0 right-0 bottom-0 p-2 bg-green-950">
                        <form
